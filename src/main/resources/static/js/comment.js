$(function () {
    ...
    function init(pageNumber) {
        Core.postAjax("/api/comments",{"sid":sid,"pageNumber": (pageNumber==null? 1 : pageNumber),"status":1},function (data) {
            var commentOne="";
            if(data.list.length==0){
                ...
                $(".comment-support").click(function () {
                    $thisLove = $(this);
                    Core.postAjax("/api/love",{"bizId":$(this).attr("biz-id"),"bizType":2},function (data) {
                        if(data.status==200){
